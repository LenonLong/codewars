...
Note:
Content is in fact not necessary in the body of the function "evaporator", you can use it or not use it, as you wish. Some people might prefer to reason with content, some other with percentages only. 
It's up to you but you must keep it as a parameter because the tests have it as an argument.
*/

function evaporator(content, evap_per_day, threshold){ 
    let currentContent = content; // Initialize current content
    let dayCount = 0; 

    // While the current content is greater than or equal to the threshold percentage
    while (currentContent >= content * threshold/100) {
        // Reduce the current content by a percentage equal to evap_per_day
